$('body').on('click', '.js-new-college', function(){
  ...
  $.post("include/insert/newCollege.php",{
    ...
    if(response == "succes"){
      ...
      M.toast({html: "Er is iets fout gegaan bij het aanmaken van een nieuw college", classes: "toast--error"});
    }
  });
})
...
$('body').on('click', '.js-new-course', function(){
  ...
  $.post("include/insert/newCourse.php",{
    ...
    if(response == "succes"){
      M.toast({html: "Nieuwe opleiding succesvol aangemaakt", classes: "toast--succes"});
    }else{
      M.toast({html: "Er is iets fout gegaan bij het aanmaken van een nieuw opleiding", classes: "toast--error"});
    }
  });
})
